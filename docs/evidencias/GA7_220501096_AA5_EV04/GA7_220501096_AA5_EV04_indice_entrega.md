# GA7_220501096_AA5_EV04 - Índice de entrega

## Proyecto

MLBT Project - María La Bonita Taquería

## Evidencia

GA7-220501096-AA5-EV04 - API del Proyecto

## Repositorio

MLBT_Proyecto

## Rama de trabajo

feature/GA7_220501096_AA5_EV04_MLBT_POSTMAN

## Módulo evaluado

api-mlbt

## Objetivo

Documentar y validar la API del proyecto MLBT mediante pruebas funcionales en Postman.

## Alcance de la evidencia

- Validación de disponibilidad de la API.
- Validación de autenticación.
- Validación de perfil protegido con token.
- Validación de usuarios.
- Validación de inventario.
- Validación de ventas.
- Validación de errores esperados.
- Exportación de colección Postman.
- Registro de capturas de evidencia.

## Estructura de evidencia

| Ruta | Propósito |
|---|---|
| api-mlbt/postman | Colección Postman de la API. |
| api-mlbt/docs/POSTMAN_EV04.md | Guía de pruebas Postman. |
| docs/evidencias/GA7_220501096_AA5_EV04 | Documentos de entrega de EV04. |
| docs/evidencias/GA7_220501096_AA5_EV04/capturas/postman | Capturas de pruebas en Postman. |
| docs/evidencias/GA7_220501096_AA5_EV04/capturas/terminal | Capturas de validaciones desde terminal. |
| docs/evidencias/GA7_220501096_AA5_EV04/resultados | Resultados y cierre de pruebas. |

## Estado

Estructura inicial creada. Las pruebas se documentarán después de validar la API en Postman.

## Resultados finales

| Elemento | Estado |
|---|---|
| Colección Postman | Completada |
| Pruebas Postman | Completadas |
| Capturas de evidencia | Completadas |
| Documento de resultados | Completado |
| Cierre técnico | Completado |

## Archivos finales

- api-mlbt/postman/GA7_220501096_AA5_EV04_MLBT_API.postman_collection.json
- api-mlbt/docs/POSTMAN_EV04.md
- docs/evidencias/GA7_220501096_AA5_EV04/GA7_220501096_AA5_EV04_plan_pruebas_postman.md
- docs/evidencias/GA7_220501096_AA5_EV04/resultados/GA7_220501096_AA5_EV04_resultados_postman.md
- docs/evidencias/GA7_220501096_AA5_EV04/resultados/GA7_220501096_AA5_EV04_cierre_tecnico.md
- docs/evidencias/GA7_220501096_AA5_EV04/capturas/postman/
